...
def get_2020(df_future_predictions, country_code, path):
...
def bokeh_plotting(df_future_predictions, country_code, df_training_predictions, dataset_train, df2020, start_time):
    actual_data = dataset_train.loc[start_time:].index
    actual = dataset_train.loc[start_time:][f'{country_code}_USD']

    actual2020_dates = df2020.loc['2020-01-02':].index
    actual2020 = df2020.loc['2020-01-02':][f'{country_code}_actual']

    pred_date = df_future_predictions.index
    predictions = df_future_predictions[f'{country_code}_USD']

    training_data = df_training_predictions.loc[start_time:].index
    training = df_training_predictions.loc[start_time:][f'{country_code}_USD']

    # create a new plot with a datetime axis type
    p = figure(plot_width=1550, plot_height=500, x_axis_type="datetime", toolbar_location="above")
